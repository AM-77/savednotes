...
export default class EditNote extends Component {
  constructor(props) {
    super(props);
    const {
      note: { title, privacy, archived, content },
    } = this.props;
    this.state = {
      noteTitle: title,
      notePrivacy: privacy,
  ...
  saveEditedNote = () => {
    ...
    const { noteTitle, notePrivacy, noteArchived, noteContent } = this.state;
          ...
          <div className="head">
            ...
            <div className="input-element">
              ...
              <input
                ...
                name="notePrivacy"
                onChange={this.onInputChange}
                defaultChecked={note.privacy === 'private'}
              />
            </div>
            <div className="input-element">
              ...
              <input
                ...
                name="noteArchived"
                onChange={this.onInputChange}
                defaultChecked={note.archived === 1}
              />
            </div>
          </div>
          <div className="body">
            <JoditEditor
              value={note.content}
              onChange={(newContent) =>
                this.setState({ noteContent: newContent })
              }
            />
          </div>
          <div className="foot">
            ...
            <button type="button" onClick={this.saveEditedNote}>
              Update Note
            </button>
          </div>
        </div>
